...
from backend.api.database.models import User, UserTicker, Ticker
...
async def first_launch(request: UsernameRequest,
...
async def get_user(request: UsernameRequest, db: AsyncSession = Depends(get_db)):
    username = request.username
...
@router.get("/{user_id}/tickers", response_model=UserTickersResponse)
async def get_user_tickers(
    ...
    # Получаем тикеры пользователя одним запросом
...
async def add_preference(
    ...
    user = (await db.execute(
        select(User).filter(User.user_id == request.user_id)
    )).scalar_one_or_none()
    ...
    existing_preference = (await db.execute(
        select(UserTicker)
        .filter(
            (UserTicker.user_id == request.user_id) &
